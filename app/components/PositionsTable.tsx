...
/** Vị thế đang giữ — giá live nến 1m, kèm nút Bán/Sửa/Xóa. */
export default function PositionsTable({ positions }: { positions: PositionLine[] }) {
  if (!positions.length) {
    return (
      <p className="card p-6 text-center text-muted">
        Chưa có vị thế nào. Bấm <b>＋ Thêm vị thế</b> hoặc <b>Đã mua</b> ở một tín hiệu.
      </p>
    );
  }
  return (
      ...
      <table className="w-full border-collapse text-xs">
        ...
        <tbody>
          {positions.map((p) => {
            ...
            return (
              <tr key={p.id} className="border-b border-border/50 last:border-0 hover:bg-white/[0.03]">
                <td className="p-3">
                  ...
                  {p.note && !p.note.startsWith("manual") && <div className="max-w-40 truncate text-[11px] text-muted">{p.note}</div>}
                </td>
                ...
                <td className={`num p-3 text-right font-medium ${tone(p.pnlPct)}`}>
                  {pct(p.pnlPct)}
                  {p.pnlVnd !== null && (
                ...
                <td className={`num p-3 text-right ${tone(p.dayPct)}`}>{pct(p.dayPct)}</td>
                <td className="num p-3 text-right text-loss">{px(p.stop)}</td>
                <td className="num p-3 text-right text-gain">{px(p.target)}</td>
                <td className="p-3 whitespace-nowrap text-muted">{p.sessionsHeld >= 2 ? "✓ bán được" : `⏳T+${p.sessionsHeld}`}</td>
                <td className="p-3">
                  <TradeActions
                    t={{ id: p.id, ticker: p.ticker, qty: p.qty, entry: p.entry, stop: p.stop, target: p.target, exit: null, status: "open", note: p.note, price: p.price }}
                  />
                </td>
              </tr>
            );
