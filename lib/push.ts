...
export type PushPayload = { title: string; body: string; url: string; tag: string; kind?: string; level?: string; force?: boolean };

// TTL giây theo loại: báo cáo định kỳ hết giá trị sau 1 vòng; tín hiệu cuối phiên phải sống qua đêm để kịp phiên sáng hôm sau.
const TTL: Record<string, number> = { positions: 900, sector: 3600, signal: 12 * 3600, stop: 6 * 3600, target: 6 * 3600, system: 12 * 3600 };
...
export async function sendPush(sub: { id: number; endpoint: string; p256dh: string; auth: string }, payload: PushPayload): Promise<number> {
  ...
  try {
    const r = await webpush.sendNotification({ endpoint: sub.endpoint, keys: { p256dh: sub.p256dh, auth: sub.auth } }, JSON.stringify(payload), {
      TTL: TTL[payload.kind ?? ""] ?? 3600,
      urgency: payload.level === "danger" ? "high" : payload.kind === "positions" ? "low" : "normal",
      timeout: 5000,
