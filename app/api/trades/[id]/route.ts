...
type Patch = { qty: string; entry: string; stop: string; target: string; exit: string; note: string; close: boolean };
...
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  if (!t) return bad("Không tìm thấy lệnh", 404);
  ...
  const opt = (v: string | undefined, cur: number | null) => (v === undefined ? cur : v === "" ? null : pos(v));
  ...
  const exit = t.status === "closed" ? (b.exit === undefined ? t.exitPrice : pos(b.exit)) : null;
  if (!qty || !Number.isInteger(qty)) return bad("Số lượng phải là số nguyên dương");
  if (!entry) return bad("Giá vốn không hợp lệ");
  if ((b.stop && !stop) || (b.target && !target) || (t.status === "closed" && !exit)) return bad("Giá không hợp lệ");
  ...
  await prisma.trade.update({
    ...
    data: {
      ...
      ...(t.status === "closed" && exit && { exitPrice: exit, pnl: netPnl(entry, exit, qty) }),
    },
  });
  return NextResponse.json({ ok: true });
}
...
export async function DELETE(_req: Request, { params }: Ctx) {
  ...
  await prisma.$transaction([
    prisma.order.updateMany({ where: { tradeId: id }, data: { tradeId: null } }),
    prisma.trade.delete({ where: { id } }),
  ]);
  if (t.status === "open") {
    await sendTelegram(`🗑 (web) Xóa vị thế #${id} <b>${t.symbol.ticker}</b> — watcher ngừng canh`).catch(() => false);
  }
  return NextResponse.json({ ok: true });
}
